...
const BasketIcon = () => {
    ...
    if (items.length === 0) return; // Don't show total aggregator if basket is empty

  return (
    <View className="absolute bottom-10 w-full z-50">
      <TouchableOpacity onPress={() => navigation.navigate('Basket')} className="flex-row mx-5 items-center space-x-1 p-4 rounded-lg bg-[#00CCBB]">
